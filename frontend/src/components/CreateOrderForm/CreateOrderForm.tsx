...
const CreateOrderForm = ({ onCloseModal }: CreateOrderFormProps) => {
  ...
  const form = useForm<{
    ...
    initialValues: { creationDate: new Date(), customerId: "", products: [], paymentMethod: "visa", vat: 0 },
    validate: (values) => ({
  ...
  const submitHandler = async (values: typeof form.values) => {
    try {
      const result = await dispatch(
        createOrder({ ...form.values, creationDate: form.values.creationDate.toISOString(), customerId: +form.values.customerId })
      ).unwrap();
